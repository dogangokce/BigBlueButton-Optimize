...
sed -i 's/clientTitle:.*/clientTitle: İdrak Academy/g' /usr/share/meteor/bundle/programs/server/assets/app/config/settings.yml

echo "Uygulama Başlığını Ayarla"
sed -i 's/appName:.*/appName: İdrak Academy/g' /usr/share/meteor/bundle/programs/server/assets/app/config/settings.yml
